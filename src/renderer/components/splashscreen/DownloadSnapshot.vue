  ...
  mounted() {
    const isDownloadSnapshot =
      remote.dialog.showMessageBox(remote.BrowserWindow.getFocusedWindow(), {
        ...
        defaultId: 0,
        detail:
          'You can change your snapshot preferences in the settings section'
      }) === 0;
  ...
  methods: {
    ...
    _getFilenameFromHeaderResponse(response) {
      ...
      return matches && matches.length > 1
        ? matches[1]
        : blockchainSnapshot.DEFAULT_FILENAME;
    },
    _resolveSnapshotDataPath(response) {
      const directoryPath = path.join(
        getWagerrDataPath(),
        blockchainSnapshot.RELATIVE_DATA_PATH
      );
      ...
      return path.join(
        directoryPath,
        this._getFilenameFromHeaderResponse(response)
      );
    },
    ...
    handleSnapshotDownloadError(err) {
      ipcRenderer.log(
        'error',
        `An error occurred while trying to download the snapshot. \n\n ${err} \n\n ${err.stack}`
      );
